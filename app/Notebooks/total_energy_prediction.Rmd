...
```{r fig.width=12}
color_values <- colorspace::diverge_hcl(n = 12, h = c(180, 260), c = c(0, 260), l = c(0,100), power = 2)
...
```
In order to try and create the most useful model, we will try to use the natural gas power generation values as predictors for the ARIMA model as well.  In order to smooth out the predictors, the rolling mean value over the last 12 months was used in addition to the actual lagged generation values.  Also, due to the COVID pandemic, we will leave out the 2020 data from training and testing (that will likely be a different analysis in the future).
...
As we can see, the differences between models is minimal, which means we should lean towards selecting the simple arima model with no predictors until we can find some significant attributes that would reduce the error.  Given that the simple model has less than a 9% mean absolute percentage error, it seems as though the decrease in consumption of coal is on a steady decline without any correlated fluctuations to the natural gas consumption.  However, note that when drift is removed from the model that the predictor becomes much more prevalent in the model.  Therefore, the usage of natural gas might be useful, if additional feature engineering is performed or other significant predictors are included that remove some of the noise in the model.  In general, there are multiple options to choose from this analysis that will give relatively good predictions, and the selection of one over the other may depend on the purposes for it's future use.